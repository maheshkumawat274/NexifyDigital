...
export default function NexifyChatbot({ onClose }: { onClose?: () => void }) {
  ...
  // ---------------------------
  const renderButtons = () => {
    const btn = "hover:bg-indigo-600 border-2 border-indigo-600 text-black hover:text-white px-4 py-2 rounded-lg shadow hover:shadow-lg transition cursor-pointer";
      ...
      <div ref={bodyRef} className="h-80 overflow-auto p-4 bg-gray-50 space-y-3">
        ...
        {isTyping && (
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 bg-indigo-600 text-white flex items-center justify-center rounded-full text-sm font-bold">ND</div>
              ...
              <div className={`flex ${isBot ? "justify-start" : "justify-end"}`}>
                ...
                <div
                  className={`max-w-[75%] px-4 py-2 rounded-lg whitespace-pre-line ${
                    isBot ? "bg-white border text-gray-800" : "bg-indigo-600 text-white"
                  }`}
                >
                  {m.text}
                </div>
